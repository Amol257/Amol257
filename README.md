...
Currently open to **Data Analyst** and **BI Analyst** roles in Delhi NCR.
...
`Excel` `Pivot Tables` `Figma` `Adobe Photoshop` `Responsive Design`
...
`Figma` `Miro` `UX Research` `WCAG Accessibility` `Agile`
...
Built an AI-powered real-time national AQI tracker aggregating live data from **511 monitoring stations** across 28 Indian states. Integrated **Gemini AI** to generate contextual health-risk insights per station alongside daily, weekly, and monthly trend views. Engineered a full-stack Python data pipeline feeding a React 19 + TypeScript frontend with interactive geospatial visualisation via Leaflet.

`React 19` `TypeScript` `Python` `Gemini AI` `Recharts` `Leaflet` `Tailwind CSS` `Vite` `REST API`

[![Live](https://img.shields.io/badge/Live_Demo-https://amol257.github.io/india-aqi-dashboard?style=flat-square&logo=vercel&logoColor=black&labelColor=0D1117)](https://amol257.github.io/india-aqi-dashboard/)
...
### PM-eBus Sewa KPI Dashboard `Sep 2025 – Nov 2025`
> **Problem:** National fleet data for 6,000+ buses lived across multiple disconnected sources with no single reporting view.

Built and deployed interactive Power BI dashboards on national government infrastructure, integrating multi-source operational data through automated Power Query pipelines. Output included Excel export functionality so stakeholders could run their own pivot-table analysis on live KPI data.

`Power BI` `DAX` `SQL` `Excel` `Power Query` `PHP` `HTML/CSS`
...
`Python` `Scikit-learn` `Pandas` `Supervised ML` `Academic Research`
...
![DAX](https://img.shields.io/badge/DAX-007ACC?style=for-the-badge&logo=microsoft&logoColor=white)
...
```
// open to work · Delhi NCR · August 2026
```
